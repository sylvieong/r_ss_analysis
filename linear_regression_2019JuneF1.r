...
df_regr <- df_all[,c('TENURE', 'HRLYEARN')] 
...
# create new dataframe which omits rows where UNION status is missing (NA)
completeVec <- complete.cases(df_all[, 'UNIONCoded'])
df_filtered <- df_all[completeVec, ]

# dev.new()
# # plot points differentiate by UNION
# print("Plotting, by union membership...", quote=FALSE)
# writeLines("\n")
# plot(ggplot(df_filtered, aes(x=TENURE, y=HRLYEARN, color=UNIONCoded, size=UNIONCoded)) +  
#      geom_point(size=1, alpha=0.5))

coeff=coefficients(linearMod)
intercept = coeff[1]
slope = coeff[2]
dev.new()
# plot points differentiate by UNION
...
plot(ggplot(df_filtered, aes(x=TENURE, y=HRLYEARN, color=UNIONCoded, size=UNIONCoded)) +  
     geom_point(size=1, alpha=0.5) + geom_abline(intercept = intercept, slope = slope))
...
dev.new()
plot(ggplot(df_filtered, aes(x=TENURE, y=HRLYEARN) ) +
  geom_bin2d(bins=100) +
  theme_bw() +
...
screen(1)
boxplot(df_filtered$HRLYEARN, main="Hourly wages", sub=paste("Outlier rows: ", length(boxplot.stats(df_all$HRLYEARN)$out)))
screen(2)
boxplot(df_filtered$TENURE, main="Job tenure (months)", sub=paste("Outlier rows: ", length(boxplot.stats(df_all$TENURE)$out)))

dev.new()
plot(ggplot(df_filtered, aes(x=UNIONCoded, y=HRLYEARN, fill=UNIONCoded)) + 
    geom_boxplot(
...
dev.new()
plot(ggplot(df_filtered, aes(x=UNIONCoded, y=TENURE, fill=UNIONCoded)) + 
    geom_boxplot(
